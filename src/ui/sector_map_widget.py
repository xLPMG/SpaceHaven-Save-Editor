...
class SectorMapWidget(QWidget):
    ...
    def _clamp_ship_position(
        self, ship: Ship, wx: float, wy: float
    ) -> tuple[float, float]:
    ...
    def _find_valid_position(
        self, ship: Ship, target_wx: float, target_wy: float
    ) -> tuple[float, float]:
    ...
    def paintEvent(self, event) -> None:
        ...
        if not self._ships:
            ...
            painter.drawText(
                self.rect(), Qt.AlignmentFlag.AlignCenter, "No sector loaded"
            )
            return
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        """Handle mouse press to start dragging a ship."""
    ...
    def mouseMoveEvent(self, event: QMouseEvent) -> None:
        """Handle mouse move to update dragging or hover state."""
        if self._dragging_ship:
            ...
            if self._drag_offset:
                ...
                new_wx_raw, new_wy_raw = self._screen_to_world(
                    new_screen_pos.x(), new_screen_pos.y()
                )
                new_wx = new_wx_raw
                ...
                self._dragging_ship.ox, self._dragging_ship.oy = world_to_ox_oy(
                    valid_wx, valid_wy
                )
                self.update()
        else:
    ...
    def mouseReleaseEvent(self, event: QMouseEvent) -> None:
        """Handle mouse release to commit the drag and emit ship_moved."""
        if event.button() == Qt.MouseButton.LeftButton and self._dragging_ship:
            self.ship_moved.emit(
                self._dragging_ship, self._dragging_ship.ox, self._dragging_ship.oy
            )
    ...
    def _end_drag(self, cursor_pos: QPointF | None) -> None:
        ...
        if cursor_pos is not None:
            ...
            self._hover_ship = ship
            self.setCursor(
                Qt.CursorShape.OpenHandCursor if ship else Qt.CursorShape.ArrowCursor
            )
        else:
            self._hover_ship = None
